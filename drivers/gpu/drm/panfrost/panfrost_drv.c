		  ...
		  struct panfrost_job *job)
{
...

    {
        ...
        u32 __user *handles = (u32 __user *)(uintptr_t)args->bo_handles;

        bos = kvmalloc_array(job->bo_count, sizeof(*bos), GFP_KERNEL | __GFP_ZERO);
        if (!bos) return -ENOMEM;

        for (i = 0; i < job->bo_count; i++) {
            u32 handle;
            if (get_user(handle, &handles[i])) { ret = -EFAULT; break; }
            bos[i] = drm_gem_object_lookup(file_priv, handle);
            if (!bos[i]) { ret = -ENOENT; break; }
        }
        if (ret) {
            for (i = 0; i < job->bo_count; i++) if (bos[i]) drm_gem_object_put_unlocked(bos[i]);
            kvfree(bos);
            return ret;
        }
        job->bos = bos;
